...
def _source_checkout_root(script_path: Path) -> Path:
    """Return the checkout root used for default benchmark resources.

    Editable installs resolve from this module's source path. Installed
    environments can still use defaults when launched from a source checkout.
    """
    candidates = (script_path.parents[3], Path.cwd())
    for candidate in candidates:
        if (
            (candidate / "benchmarks" / "viewer-benchmark-scenario.v1.json").is_file()
            and (candidate / "benchmarks" / "viewer-thresholds.v1.json").is_file()
        ):
            return candidate
    return candidates[0]
...
_REPOSITORY_ROOT = _source_checkout_root(_SCRIPT_PATH)
