...
install_desktop_integration() {
  ...
  find "$icons_hicolor_dir" \
    \( -path "*/apps/${application_id}.png" \
       -o -path "*/apps/${application_id}.svg" \
       -o -path "*/apps/${application_id}-symbolic.svg" \) \
  ...
  if [ "$debug" = "1" ] || [ "$install_only" = "1" ]; then
    ...
    echo "[CaveViewer AppRun] Desktop icons: $icons_hicolor_dir/*/apps/${application_id}*"
  fi
}
...
uninstall_desktop_integration() {
  ...
  echo "[CaveViewer AppRun] Removed desktop icons: $icons_hicolor_dir/*/apps/${application_id}*"
}
