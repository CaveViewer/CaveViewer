...
| `CAVEVIEWER_TK_SCALE` | _(display DPI)_ | Development-only Windows/Linux override in Tk pixels per point, clamped to `0.75` through `4.0`; it is not an OS scaling percentage or user preference. Normal Windows startup uses the window's native effective DPI, while the Linux AppImage launcher normally derives this value from the desktop Xft DPI setting. |
