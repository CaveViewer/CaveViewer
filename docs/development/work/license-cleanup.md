...
| 8 | Clean up any failed build or release workflow. | License and packaging-contract changes may expose a build or package-smoke failure that could recur if repaired without regression coverage. | PR #306 run `32658467620`, job `97240883928`, failed in “Validate pull-request release metadata.” The validator treated every change to the AppStream file—including the project-license correction—as a release-entry change and incorrectly required a version bump and signed manifests. All platform packaging and other source checks passed. | The gate compares only the AppStream `<releases>` history when deciding whether synchronized release metadata is required. Non-release fields such as `project_license` may change independently, while an actual release-entry change still requires `version.py`, a single prepended entry, and signed manifests. Behavioral tests cover license-only changes, release-entry changes, and missing release metadata. | Preserve the existing version, manifest-signature, branch, and packaging gates.<br>Move AppStream release-history comparison into a focused reusable helper.<br>Use that helper in the pull-request validator instead of classifying the whole metainfo file as a release entry.<br>Add regression tests proving the AGPL license-only edit is accepted and a new release entry remains classified as release metadata.<br>Run focused and complete local tests, push the repair, and verify the replacement PR checks after waiting at least one minute between status checks. | `chore/agpl-license-cleanup` | Fix implemented locally — CI pending |
...
- Pull request [#306](https://github.com/CaveViewer/CaveViewer/pull/306) is open.
  Run `32658467620`, job `97240883928`, exposed an over-broad AppStream
  release-change classifier; all other source and platform packaging checks
  passed. The focused repair is implemented and awaiting a replacement CI run.
...
- `.venv-dev/bin/python -m pytest -p no:cacheprovider -q tests/unit/test_appstream_release_metadata.py tests/unit/test_release_workflows.py tests/unit/test_license_contract.py` — 41 passed after the workflow repair.
- `.venv-dev/bin/python -m pytest -p no:cacheprovider -q` — 1856 passed after
  the workflow repair, with one third-party pending-deprecation warning.
...
- Replacement PR validation remains pending after the workflow repair is
