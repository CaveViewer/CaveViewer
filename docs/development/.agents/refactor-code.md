...
| 11 | Map Library cache-rebuild orchestration | `refactor/map-library-cache-rebuild` | Implemented locally — review pending | Enforce after merge | `70c28ca` | [#229](https://github.com/CaveViewer/CaveViewer/issues/229) | A focused orchestration owner wraps `CacheRebuildJobController` and owns the Tk polling token, pause/close requests, and typed update delivery without duplicating process state. |
