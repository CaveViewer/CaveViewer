...
| 8 | Splash controller lifecycle | `refactor/splash-controller-lifecycle` | Not implemented — tracked | Applies, after workstream 9 | — | [#226](https://github.com/CaveViewer/CaveViewer/issues/226) | `SplashSession` already owns callback cancellation and close/result state; build the controller around it rather than introducing a second lifecycle owner. |
| 9 | Map Library workflow dependencies | `refactor/map-library-workflow-dependencies` | Not implemented — tracked | Applies; next structural branch | — | [#227](https://github.com/CaveViewer/CaveViewer/issues/227) | `MapLibraryWorkflow` remains 1,967 lines and accepts roughly 40 loose constructor dependencies. Introduce typed bundles before controller extraction. |
| 10 | Map Library catalog and download workflows | `refactor/map-library-catalog-download` | Not implemented — tracked | Applies | — | [#228](https://github.com/CaveViewer/CaveViewer/issues/228) | The coordinator still owns catalog polling/reconciliation and active-download queues, cancellation, inhibition, and completion. Keep catalog and download as separate owners within the branch. |
| 11 | Map Library cache-rebuild orchestration | `refactor/map-library-cache-rebuild` | Not implemented — tracked | Applies | — | [#229](https://github.com/CaveViewer/CaveViewer/issues/229) | Reuse `CacheRebuildJobController`; extract Tk polling, presentation, notification, and teardown ownership without duplicating process state. |
| 12 | Splash composition cleanup | `refactor/splash-composition-cleanup` | Not implemented — tracked | Applies only after 8, 10, and 11 | — | [#230](https://github.com/CaveViewer/CaveViewer/issues/230) | `splash_screen.py` remains 2,377 lines and `show_splash_screen()` still combines composition with nested workflow callbacks. Delete facades only after direct composition is ready. |
...
| 16 | Runtime-settings fallback retirement | `refactor/runtime-settings-fallback-retirement` | Implemented locally — review pending | Enforce after merge | `ed6dded` | [#268](https://github.com/CaveViewer/CaveViewer/issues/268) | Remaining direct environment access is classified by ownership and an architecture test requires every module-level exception to stay explicitly allowlisted. |
| 17 | Viewer-window composition boundary | `refactor/viewer-window-composition-boundary` | Suggested — tracked | Applies after splash ownership stabilizes | — | [#269](https://github.com/CaveViewer/CaveViewer/issues/269) | `viewer_window.py` is 7,924 lines and mixes composition, benchmark configuration, map loading, validation, capture, recording, and interaction coordination. Extract one characterized owner per branch rather than performing a wholesale rewrite. |
