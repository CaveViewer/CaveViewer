...
| `CAVEVIEWER_WINDOW_SYSTEM` | `auto` | Linux viewer backend: `auto` prefers native Wayland and retries X11 on a recognized initialization failure; `wayland` and `x11` require that protocol without fallback. The Linux AppImage launcher defaults this to `x11` on Wayland sessions when `DISPLAY` is available so GNOME/XWayland supplies normal titlebar and resize decorations; set `CAVEVIEWER_WINDOW_SYSTEM=wayland` to force native Wayland for AppImage testing. |
