...
def test_validated_mesh_path_graph_rechecks_every_persisted_edge():
    points = (
        (0.25, 0.125, 0.25),
        (0.75, 0.125, 0.25),
        (1.25, 0.125, 0.25),
    ...
    result = build_validated_mesh_path_graph(
        points,
        lattice_spacing_m=(0.5, 0.25, 0.5),
        footprint_cell_size_m=1.0,
        point_probe=lambda _point: (True, 1.0),
        edge_is_clear=lambda first, second: (
            checked_edges.append((first, second)) or True
        ),
        minimum_clearance_m=0.25,
    )

    assert result.graph is not None
    assert result.details["reason"] == "validated_ordered_mesh_path_built"
    assert len(checked_edges) == 2
    assert result.graph.terminal_count == 1
    assert result.graph.nodes[(2, 0, 0)].terminal is True


def test_validated_mesh_path_graph_rejects_a_revisited_voxel():
    result = build_validated_mesh_path_graph(
        (
            (0.25, 0.125, 0.25),
            (0.75, 0.125, 0.25),
    ...
    assert result.details["reason"] == "validated_mesh_path_revisit_detected"


def test_exact_cubic_spine_reroutes_a_mesh_rejected_cardinal_edge():
    graph = SparseCubicVoxelGraph.from_keys(
        tuple((x, 0, 0) for x in range(3))
        + tuple((x, 0, 1) for x in range(3))
    )
    rejected = {
        frozenset(
            (
                graph.voxel_center((1, 0, 0)),
                graph.voxel_center((2, 0, 0)),
            )
        )
    }

    result = build_exact_cubic_spine_navigation_path_graph(
        graph,
        (graph.voxel_center((0, 0, 0)), graph.voxel_center((2, 0, 0))),
        start_keys=((0, 0, 0),),
        terminal_keys=((2, 0, 0),),
        footprint_cell_size_m=8.0,
        point_probe=lambda point: (
            (True, 1.0) if graph.contains_key(graph.world_key(point)) else None
        ),
        edge_is_clear=lambda first, second: frozenset((first, second))
        not in rejected,
        horizontal_gate_radius_m=1.0,
        config=MeshNavigationGraphConfig(
            horizontal_sample_spacing_m=1.0,
            vertical_sample_spacing_m=1.0,
            max_nodes=64,
            max_edge_distance_m=1.1,
            max_vertical_edge_distance_m=1.1,
        ),
    )

    assert result.graph is not None
    assert result.details["reason"] == (
        "exact_cubic_spine_terminal_path_built"
    )
    assert result.details["mesh_rejected_edge_count"] == 1
    assert result.details["repair_pass_count"] == 1
    assert (1, 0, 1) in result.graph.nodes
    assert result.graph.terminal_count == 1
    assert result.graph.nodes[(2, 0, 0)].terminal is True


def test_exact_cubic_spine_rejects_an_occupied_boundary_sample():
    graph = SparseCubicVoxelGraph.from_keys(
        tuple((x, 0, 0) for x in range(3))
    )

    def point_probe(point):
        if math.isclose(point[0], 2.0, rel_tol=0.0, abs_tol=1e-9):
            return None
        return True, 1.0
    ...
    assert result.details["edge_voxel_rejection_count"] >= 1


def test_exact_cubic_spine_publishes_no_prefix_when_all_terminal_edges_blocked():
    graph = SparseCubicVoxelGraph.from_keys(
        tuple((x, 0, z) for x in range(3) for z in range(2))
    )

    def terminal_wall(first, second):
        return not (
            min(first[0], second[0]) < 2.0 < max(first[0], second[0])
        )
    ...
    assert result.graph is None
    assert result.details["reason"] == (
        "exact_cubic_spine_terminal_unreachable"
    )
    assert result.details["known_terminal_reached"] is False
    assert result.details["mesh_rejected_edge_count"] == 2


def test_exact_cubic_spine_simplifies_only_through_exact_approved_edges():
    graph = SparseCubicVoxelGraph.from_keys(
        tuple((x, 0, 0) for x in range(11))
    )
    approved: set[frozenset[tuple[float, float, float]]] = set()

    def approve(first, second):
        approved.add(frozenset((first, second)))
        return True

    result = build_exact_cubic_spine_navigation_path_graph(
        graph,
        (graph.voxel_center((0, 0, 0)), graph.voxel_center((10, 0, 0))),
        start_keys=((0, 0, 0),),
        terminal_keys=((10, 0, 0),),
        footprint_cell_size_m=16.0,
        point_probe=lambda _point: (True, 1.0),
        edge_is_clear=approve,
        horizontal_gate_radius_m=1.0,
        config=MeshNavigationGraphConfig(
            horizontal_sample_spacing_m=1.0,
            vertical_sample_spacing_m=1.0,
    ...
    for source_key, edges in result.graph.edges.items():
        for edge in edges:
            assert frozenset(
                (
                    result.graph.nodes[source_key].center,
                    result.graph.nodes[edge.target].center,
                )
            ) in approved
    assert result.graph.terminal_count == 1


def test_exact_cubic_spine_simplification_preserves_ordered_gate_witnesses():
    keys = (
        tuple((x, 0, 0) for x in range(5))
        + tuple((4, 0, z) for z in range(1, 5))
        + tuple((x, 0, 4) for x in range(3, -1, -1))
    )
    ...
    result = build_exact_cubic_spine_navigation_path_graph(
        graph,
        (
            graph.voxel_center((0, 0, 0)),
            graph.voxel_center(first_gate),
            graph.voxel_center(second_gate),
            graph.voxel_center(terminal),
        ),
        start_keys=((0, 0, 0),),
        terminal_keys=(terminal,),
        footprint_cell_size_m=16.0,
        point_probe=lambda _point: (True, 1.0),
        edge_is_clear=lambda _first, _second: True,
        horizontal_gate_radius_m=1.0,
        waypoint_key_groups=((first_gate,), (second_gate,)),
    ...
    assert result.graph is not None
    assert first_gate in result.graph.nodes
    assert second_gate in result.graph.nodes
    assert result.details["cubic_spine"][
        "persisted_intermediate_gate_witness_count"
    ] == 2
    assert result.details["terminal_graph_distance_m"] == 12.0
...
def test_goal_directed_mesh_graph_uses_reachable_endpoint_snap_candidate():
    free_keys = {(x, 0, 0) for x in range(5)}
    blocked_primary = (5.5, 0.5, 0.5)
    ...
    result = build_goal_directed_seeded_mesh_navigation_path_graph(
        ...
        component_cells=((0, 0),),
        point_probe=_grid_probe(free_keys, spacing_m=1.0),
        edge_is_clear=lambda _first, second: tuple(second) != blocked_primary,
        terminal_point=blocked_primary,
        terminal_candidate_points=(reachable_candidate,),
        route_guide_points=((0.5, 0.5, 0.5), blocked_primary),
        config=MeshNavigationGraphConfig(
            horizontal_sample_spacing_m=1.0,
            vertical_sample_spacing_m=1.0,
            max_nodes=64,
    ...
    assert result.details["selected_terminal_hint_index"] == 1
...
def test_goal_directed_mesh_graph_ignores_zero_edge_terminal_candidate():
    free_keys = {(x, 0, 0) for x in range(6)}
    ingress_candidate = (0.5, 0.5, 0.5)
    ...
    result = build_goal_directed_seeded_mesh_navigation_path_graph(
        ...
        point_probe=_grid_probe(free_keys, spacing_m=1.0),
        edge_is_clear=lambda _first, _second: True,
        terminal_point=primary_terminal,
        terminal_candidate_points=(ingress_candidate,),
        route_guide_points=(ingress_candidate, primary_terminal),
        config=MeshNavigationGraphConfig(
            horizontal_sample_spacing_m=1.0,
            vertical_sample_spacing_m=1.0,
            max_nodes=64,
    ...
    assert result.details["persisted_path_node_count"] >= 2
    assert result.graph.terminal_count == 1
...
def test_goal_directed_mesh_graph_rejects_blocked_local_edge_midpoint():
    ...
    assert result.details["non_improving_edge_skip_count"] > 0
...
def test_goal_directed_mesh_graph_probes_exact_diagonal_boundary():
    fractions = segment_voxel_probe_fractions(
        (0.5, 0.5, 0.5),
        (1.5, 0.5, 1.5),
        lattice_spacing_m=1.0,
        maximum_sample_spacing_m=0.5,
    )

    assert 0.5 in fractions
    assert fractions == tuple(sorted(set(fractions)))
    assert all(0.0 < fraction < 1.0 for fraction in fractions)


def test_goal_directed_mesh_graph_preserves_quarter_metre_vertical_lattice():
    free_keys = {(x, 1, 0) for x in range(5)}

    def point_probe(point):
        key = (
            int(math.floor(point[0])),
            int(math.floor(point[1] / 0.25)),
            int(math.floor(point[2])),
        )
        return (True, 0.25) if key in free_keys else None
    ...
    result = build_goal_directed_seeded_mesh_navigation_path_graph(
        ...
        point_probe=point_probe,
        edge_is_clear=lambda _first, _second: True,
        terminal_point=(4.5, 0.375, 0.5),
        route_guide_points=((0.5, 0.375, 0.5), (4.5, 0.375, 0.5)),
        config=MeshNavigationGraphConfig(
            horizontal_sample_spacing_m=1.0,
    ...
    assert result.graph is not None
    assert result.graph.grid_size_m == (1.0, 0.25, 1.0)
    assert result.details["lattice_spacing_m"] == [1.0, 0.25, 1.0]
    assert all(node.center[1] == 0.375 for node in result.graph.nodes.values())
    assert result.graph.terminal_count == 1


def test_goal_directed_mesh_graph_does_not_trust_footprint_guide_height():
    free_keys = {(x, 1, 0) for x in range(7)}
    ...
    assert result.graph is not None
    assert result.details["search_heuristic"] == (
        "ordered_horizontal_route_remaining_distance"
    )
    assert result.details["maximum_route_guide_index_seen"] == 2
    assert all(node.center[1] == 0.375 for node in result.graph.nodes.values())
