...
def test_load_bookmarks_keeps_only_valid_slots(tmp_path):
    bookmarks_path = tmp_path / "bookmarks.json"
    bookmarks_path.write_text(
        """
        {
          "slots": {
            "1": {"position": [1, 2, 3], "yaw": 0.5, "pitch": -0.25},
            "0": {"position": [9, 9, 9], "yaw": 1, "pitch": 1},
            "10": {"position": [9, 9, 9], "yaw": 1, "pitch": 1},
    ...
    window = object.__new__(viewer_window.CaveViewerWindow)
    window._bookmarks_path = str(bookmarks_path)

    window._load_bookmarks()

    assert window._bookmarks == {
        1: {"position": [1.0, 2.0, 3.0], "yaw": 0.5, "pitch": -0.25}
    }


def test_recall_bookmark_clamps_pitch_and_moves_to_nearest_safe_position():
    shown = []
    safe_position = np.array([4.0, 5.0, 6.0], dtype=np.float64)
    window = object.__new__(viewer_window.CaveViewerWindow)
    window._has_map_loaded = True
    window._bookmarks = {
        2: {"position": [99.0, 0.0, 0.0], "yaw": 1.5, "pitch": 8.0}
    }
    ...
    window._navigation_position_is_allowed = lambda _position: False
    window._nearest_navigation_guard_position = lambda _position: safe_position
    window.controls_overlay = SimpleNamespace(
        show_panel=lambda: shown.append("panel")
    )

    assert window._recall_bookmark_slot(2) is True

    assert np.array_equal(window.camera.position, safe_position)
    assert window.camera.yaw == 1.5
    assert window.camera.pitch == 1.2
    assert window.camera.roll == 0.0
    assert shown == ["panel"]


def test_bookmark_hotkey_delete_precedes_save_modifier(monkeypatch):
    monkeypatch.setattr(viewer_window.sys, "platform", "linux")
    calls = []
    window = object.__new__(viewer_window.CaveViewerWindow)
    window._has_map_loaded = True
    window._keys_down = {127}
    window._key_resolve_cache = {}
    window.wnd = SimpleNamespace(keys=SimpleNamespace(KEY_1=49, DELETE=127))
    window._platform_adapter = SimpleNamespace(
        bookmark_save_modifier=lambda: "control"
    )
    window._delete_bookmark_slot = lambda slot: calls.append(("delete", slot))
    window._save_bookmark_slot = lambda slot: calls.append(("save", slot))
    window._recall_bookmark_slot = lambda slot: calls.append(("recall", slot))

    assert window._handle_bookmark_hotkey(49, SimpleNamespace(ctrl=True)) is True
